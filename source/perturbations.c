...
int perturb_workspace_init(
  ...
  int index_ap;
  int l;
  // double w_gdm, dw_over_da_gdm, integral_gdm; // TK uncomment this if l3_gdm and l_max_gdm is wanted 
  ...
  if (_scalars_) {
    ...
    //   if (w_gdm > 0.33) {
    //     ppw->max_l_max = MAX(ppw->max_l_max, ppr->l_max_gdm);
    //   }
    // }
...
int perturb_vector_init(
  ...
  // TK added GDM w_gdm here to be retrieved from background in case we want things to depend on w_gdm or l3_gdm 
...
int perturb_derivs(double tau,
  ...
  double delta_b,theta_b;
  double cb2,cs2,ca2,cg2; // TK added cg2 here for the GDM adiabatic sound speed
    ...
    if (pba->has_gdm == _TRUE_) {
      ...
      // w(t) equations 
        ...
        if(isnan(dy[pv->index_pt_delta_gdm])){
          ...
          dy[pv->index_pt_theta_gdm] =
            ...
            k2*( ppt->ceff2_gdm*y[pv->index_pt_delta_gdm]/(1 + w_gdm) -s2_squared*y[pv->index_pt_shear_gdm]) 
      ...
      // }
        ...
        /** - -----> TK : Or following ur species code for fluid approximations "a la Hu"
          ...
          Also assume sigma^dot = 2/3 w/(1+w) pi_g^dot + 2/3 pi_g w^dot/(1+w)^2 
          i.e. the time evolution of pi and sigma have w^dot dependence 
          Eq. (12) in Hu the becomes:
          sigma^dot = sigma w^dot / w / (1+w) - 3 a^dot / a sigma + 8/3 cvis^2 / (1+w) (theta + metric_shear)
          */
